...
//
//*******************************************************************************************
class UIFacility_InterrogationFacility extends UIFacility; //UIFacility_PsiLab;
...
var localized string m_strCancelInRProjectTitle;
var localized string m_strCancelInRProjectBody;
...
{
	local UIChooseInRProject kScreen;
	...
	if (NeedResearchReportPopup(TechRefs))
	{
		...
		{
			kScreen = `HQPRES.Spawn(class'UIChooseInRProject', `HQPRES);
...
{
	AddFacilityButton(m_strStartProject, OnChooseInRProject);
	AddFacilityButton(m_strCheckInventory, OnCheckInventory);
	
}

simulated function OnChooseInRProject()
{
	local UIChooseInRProject kScreen;
...
{
	local UIInventory_InR kScreen;

	if (`HQPRES.ScreenStack.IsNotInStack(class'UIInventory_InR'))
	{
		kScreen = `HQPRES.Spawn(class'UIInventory_InR', `HQPRES);
		`HQPRES.ScreenStack.Push(kScreen,`HQPRES.Get3DMovie());
...
simulated function String GetProgressString()
{
	...
	InRProject = XComGameState_HeadquartersProjectInR(`XCOMHISTORY.GetGameStateForObjectID(class'XComGameState_HeadquartersProjectInR'.static.GetCurrentInRProject().ObjectID));
	
	if(InRProject != none)
	{
		return m_strProgress @ class'UIUtilities_Strategy'.static.GetResearchProgressString(XCOMHQ().GetResearchProgress(class'XComGameState_HeadquartersProjectInR'.static.GetCurrentInRProject() )); //.GetReference()) );
	}
	else
	{
		return "";
...
{
	return class'UIUtilities_Strategy'.static.GetResearchProgressColor(XCOMHQ().GetResearchProgress(class'XComGameState_HeadquartersProjectInR'.static.GetCurrentInRProject() )); //.GetReference()) );
}
...
simulated function UpdateBuildProgress()
{
	...
	if(InRProject != none)
	{
		...
		m_BuildProgress.Update(m_strCurrentProject, InRTech.GetMyTemplate().DisplayName, days, progress, int(100 * InRProject.GetPercentComplete()));
...
simulated function bool IsProjectStalled()
{
	...
	if(InRProject != none)
	{
		return InRProject.GetCurrentNumHoursRemaining() < 0;
	}
	else
	{
		return false;
...
simulated function OnUpButtonClicked(int ListItemIndex)
{
	...
	NewGameState = class'XComGameStateContext_ChangeContainer'.static.CreateChangeState("Edit InR Build Queue: Move Item Up");
...
simulated function OnDownButtonClicked(int ListItemIndex)
{
	...
	NewGameState = class'XComGameStateContext_ChangeContainer'.static.CreateChangeState("Edit InR Build Queue: Move Item Down");
	NewFacilityState = XComGameState_FacilityXCom(NewGameState.ModifyStateObject(class'XComGameState_FacilityXCom', FacilityRef.ObjectID));
...
simulated function CancelProjectPopup(int ListItemIndex)
{
	...
	ParamTag.StrValue0 = InRTech.GetDisplayName();

	kData.strTitle = m_strCancelInRProjectTitle;
	kData.strText = `XEXPAND.ExpandString(m_strCancelInRProjectBody);
...
simulated function OnCancelProjectPopupCallback(Name eAction, UICallbackData xUserData)
{
	...
	if (eAction == 'eUIAction_Accept')
	{
		...
		NewGameState = class'XComGameStateContext_ChangeContainer'.static.CreateChangeState("Edit InR Build Queue: Remove Project");
